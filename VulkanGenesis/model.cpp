...
// include and define in .cpp otherwise it gets included multiple times, giving an LNK2005 error.
#define TINYOBJLOADER_IMPLEMENTATION
#include <tiny_obj_loader.h>

namespace std {
	template <>
	struct hash<Lorenz::Model::Vertex> {
		size_t operator()(Lorenz::Model::Vertex const& vertex) const {
			size_t seed = 0;
			Lorenz::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
			return seed;
		}
	};
}  // namespace std
...
namespace Lorenz
{
	...
	Model::Model(LorenzDevice& device, const Model::Builder& builder) : lorenzDevice(device)
	{
	...
	std::unique_ptr<Model> Model::createModelFromFile(
		LorenzDevice& device, const std::string& filepath) {
		Builder builder{};
		builder.loadModel(filepath);
		std::cout << "Number of vertices: " << builder.vertices.size() << std::endl;
