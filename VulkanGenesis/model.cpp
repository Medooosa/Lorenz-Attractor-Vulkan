...
namespace Lorenz
{
	...
	Model::Model(LorenzDevice& device, const std::vector<Vertex>& vertices) : lorenzDevice(device)
	{
	...
	void Model::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		void* data;
		vkMapMemory(lorenzDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(lorenzDevice.device(), vertexBufferMemory);
	}
